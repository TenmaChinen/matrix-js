...
QUnit.module("Correct Shape", function () {
  QUnit.test("Matrix Asymmetric", function (assert) {
    assert.throws(()=>new Matrix([[1,2,3],[4,5]]));
  });
  QUnit.test("Matrix Not Array", function (assert) {
    assert.throws(()=>new Matrix("String"));
  });

  QUnit.test("Matrix Is Number", function (assert) {
    assert.throws(()=>new Matrix(7));
  });
});
